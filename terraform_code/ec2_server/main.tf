...
resource "tls_private_key" "DevSecOps-keypair" {
  algorithm = "RSA"
  rsa_bits  = 2048
}
...
resource "local_file" "private_key" {
  content  = tls_private_key.DevSecOps-keypair.private_key_pem
  filename = "${path.root}/DevSecOps-key-pair.pem"
}

# Create an AWS Key Pair using the generated public key
resource "aws_key_pair" "DevSecOps-keypair" {
  key_name   = "DevSecOps-key-pair"
  public_key = tls_private_key.DevSecOps-keypair.public_key_openssh
}

resource "aws_instance" "my-ec2" {
  ami           = var.ami   
  instance_type = var.instance_type
  key_name      = aws_key_pair.DevSecOps-keypair.key_name       
  ...
  provisioner "remote-exec" {
    ...
    connection {
      type        = "ssh"
      private_key = tls_private_key.DevSecOps-keypair.private_key_pem  ##file("${path.root}/DevSecOps-key-pair.pem") # replace with your key-name 
      user        = "ubuntu"
      host        = self.public_ip
    }
